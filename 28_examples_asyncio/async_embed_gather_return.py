...
async def main():
    ...
    # 得到的就是一个结果的列表
    return await asyncio.gather(*tasks) # 不在main协程函数里处理结果，直接返回await的内容，那么最外层的run_until_complete将会返回main协程的结果
...
# 协程对象需要注册到事件循环，由事件循环调用
# 嵌套调用，main函数嵌套调用协程 do_work，接收main协程的返回结果
results = loop.run_until_complete(main())   # 协程方法运行，协程方法 do_work 输出运行结果

...
try:
    ...
    # 嵌套调用，main函数嵌套调用协程 do_work
    # 注意需要两个变量接收返回值
    dones, pendings = loop.run_until_complete(main_task)  # 协程方法运行，协程方法 do_work 输出运行结果
...
# 在 terminal 终端在启动项目，然后按下 Ctrl + C 结束，触发 run_until_complete的执行异常 KeyboardInterrupt
    ...
    for task in asyncio.Task.all_tasks():
        # 取消 main 协程任务，就取消了所有 main 协程中的子任务
        if "main" in str(task):
            # 取消协程任务
...
# output:
# ---------------------------------------------------------------------------
# task:  [<Task pending coro=<do_work() running at async_cancel_main.py:47>>, <Task pending coro=<do_work() running at async_cancel_main.py:47>>, <Task pending coro=<do_work() running at async_cance
# l_main.py:47>>]
# waiting:  1
# waiting:  3
# waiting:  4

...
# 嵌套调用，main函数嵌套调用协程 do_work
# 注意需要两个变量接收返回值
dones, pendings = loop.run_until_complete(main())   # 协程方法运行，协程方法 do_work 输出运行结果
...
# output:
# ---------------------------------------------------------------------------
# task:  [<Task pending coro=<do_work() running at D:/work_private/python-examples/28_examples_asyncio/async_embed_wait_return.py:42>>, <Task pending coro=<do_work() running at D:/work_private/python-examples/28_examples_asyncio/async_embed_wait_return.py:42>>, <Task pending coro=<do_work() running at D:/work_private/python-examples/28_examples_asyncio/async_embed_wait_return.py:42>>]
# waiting:  1
# waiting:  3
# waiting:  4
